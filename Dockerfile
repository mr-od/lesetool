# Multi-stage build for Directus with custom extensions
FROM node:18-alpine AS extension-builder

# Set working directory
WORKDIR /app

# Copy package files
COPY package*.json ./
COPY extensions/ ./extensions/

# Install dependencies and build all extensions
RUN npm ci

# Build each extension
RUN cd extensions/endpoint-energy-summary && npm ci && npm run build
RUN cd extensions/endpoint-sql-view && npm ci && npm run build  
RUN cd extensions/endpoint-run-battery-sim && npm ci && npm run build
RUN cd extensions/panel-energy-summary && npm ci && npm run build
RUN cd extensions/panel-sql-view && npm ci && npm run build
RUN cd extensions/panel-run-battery-sim && npm ci && npm run build
RUN cd extensions/panel-open-link && npm ci && npm run build
RUN cd extensions/interface-dropdown && npm ci && npm run build

# Production stage
FROM directus/directus:11.9.2

# Copy built extensions
COPY --from=extension-builder /app/extensions/endpoint-energy-summary/dist /directus/extensions/endpoint-energy-summary
COPY --from=extension-builder /app/extensions/endpoint-sql-view/dist /directus/extensions/endpoint-sql-view
COPY --from=extension-builder /app/extensions/endpoint-run-battery-sim/dist /directus/extensions/endpoint-run-battery-sim
COPY --from=extension-builder /app/extensions/panel-energy-summary/dist /directus/extensions/panel-energy-summary
COPY --from=extension-builder /app/extensions/panel-sql-view/dist /directus/extensions/panel-sql-view
COPY --from=extension-builder /app/extensions/panel-run-battery-sim/dist /directus/extensions/panel-run-battery-sim
COPY --from=extension-builder /app/extensions/panel-open-link/dist /directus/extensions/panel-open-link
COPY --from=extension-builder /app/extensions/interface-dropdown/dist /directus/extensions/interface-dropdown

# Copy database schema files
COPY tables_up.sql /docker-entrypoint-initdb.d/01-schema.sql
COPY materialized_view.sql /docker-entrypoint-initdb.d/02-views.sql
COPY summary.sql /docker-entrypoint-initdb.d/03-summary.sql

# Copy uploads directory if needed
COPY uploads/ /directus/uploads/

# Set proper permissions
USER root
RUN chown -R node:node /directus/extensions /directus/uploads
USER node